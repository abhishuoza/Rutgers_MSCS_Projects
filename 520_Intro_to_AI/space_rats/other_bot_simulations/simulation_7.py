...
BLOCKED, OPEN, BOT, RAT = 0, 1, 2, 3

# For coloring grid plot
cmap = ListedColormap(["black", "grey", "blue", "green"])
bounds = [BLOCKED - 0.5, OPEN - 0.5, BOT - 0.5, RAT - 0.5, RAT + 0.5]
...
def bot1(grid, bot_pos, rat_pos, alpha):
    ...
    while len(possible_locations) > 1:
        ...
        open_direction_totals = np.array([0, 0, 0, 0])
        for location, open_directions in possible_locations.items():
            open_direction_totals = open_direction_totals + open_directions
        direction_index = random.choice([0, 1, 2, 3])
        match direction_index:
            case 0:
                decided_pos = (bot_pos[0] - 1, bot_pos[1])
            case 1:
                decided_pos = (bot_pos[0], bot_pos[1] - 1)
            case 2:
                decided_pos = (bot_pos[0] + 1, bot_pos[1])
            case 3:
                decided_pos = (bot_pos[0], bot_pos[1] + 1)
            case _:
                raise Exception("This index should not exist in direction_index")
        ...
        if move_success:
            ...
            match direction_index:
                case 0:
                    possible_locations = {(i - 1, j): [is_open(grid, i - 1 - 1, j), is_open(grid, i - 1, j - 1),
                                                       is_open(grid, i - 1 + 1, j), is_open(grid, i - 1, j + 1)] for
                                          i, j in possible_locations.keys()}
                case 1:
                    possible_locations = {(i, j - 1): [is_open(grid, i - 1, j - 1), is_open(grid, i, j - 1 - 1),
                                                       is_open(grid, i + 1, j - 1), is_open(grid, i, j - 1 + 1)] for
                                          i, j in possible_locations.keys()}
                case 2:
                    possible_locations = {(i + 1, j): [is_open(grid, i + 1 - 1, j), is_open(grid, i + 1, j - 1),
                                                       is_open(grid, i + 1 + 1, j), is_open(grid, i + 1, j + 1)] for
    ...
    if bot_pos not in possible_locations:
        raise Exception("Location not found by bot.")
    else:
        print("Bot location found!")
...
for test in range(1000):
    ...
    new_row_df.to_csv("results_7.csv", mode='a', header=False, index=False)
    # plt.imshow(ship1, cmap=cmap, norm=norm)
    # plt.title("Ship config")
    # plt.show()
