...
BLOCKED, OPEN, BOT, RAT, BOT_RAT = 0, 1, 2, 3, 4 # BOT_RAT means both bot and rat are on that cell
...
# # Bot actions performable by the bot - sense_neighbors, detect_rat_ping and bot_attempt_move
...
def bot_attempt_move(grid, bot_pos, decided_pos):
    ...
    if  decided_pos[0] < 0 or decided_pos[0] >= size or decided_pos[1] < 0 or decided_pos[1] >= size:
        ...
        if grid[bot_pos] == BOT_RAT:
            grid[bot_pos] = RAT # If the cell has both BOT and RAT, leave the RAT behind
        else:
            grid[bot_pos] = OPEN
        bot_pos = decided_pos
        if grid[bot_pos] == RAT:
            grid[bot_pos] = BOT_RAT # If destination cell already contains RAT, then mark it as BOT_RAT
        else:
            grid[bot_pos] = BOT
        return bot_pos, 1
...
# This function is for phase 1 to create set of possible locations
...
# We can end up doing a lot more beeps than 2 * size,
# so this stores all possible beep_probabilities in beep_Ps 
...
# For finding path to decided location
...
# Rat probabilities update function
def update_Ps(p_rat, beep, beep_p, bot_pos, rat_pos):
    # Entrirely a bayesian update with making prob value of current cell as 0.
    # Rat found case is covered in bot function
    if beep == 1:
...
def bot1(grid, bot_pos, rat_pos, alpha):
    ...
    # Phase 1
    # Creating set of locations where the bot could be
    possible_locations = {}
    for i in range(size):
        for j in range(size):
    ...
    while len(possible_locations) > 1:
        ...
        # Move in a random open location
        ...
        # Update using information of move success or failure
        if move_success:
            possible_locations = {location: open_directions
                                  for location, open_directions in possible_locations.items()
    ...
    # Sense to break the symmetry of p_rat to find a location with max prob  
    ...
    while True:
        ...
        # Plan path to location with max prob
        max_loc = max(p_rat, key=p_rat.get)
        path, path_found = astar_path(grid, bot_pos, max_loc)

        # Keep moving and sensing untill location is reached
...
def bot2(grid, bot_pos, rat_pos, alpha):
    ...
    # Make a lot of sensings to create a fuzzy radius
    ...
    while True:
        ...
        # Make multiple moves and multiple sensings to refine p_rat with more fuzzy radii
...
for test in range(1000):
    ...
    ship2 = ship1.copy()
    # Testing on 10 linspaced values of alpha between 0 and 0.2
