"""
This code is the same as without rat moving, witht these changes - 
    move_rat() which is called to move the rat after each action taken by the bots
    update_Ps_after_rat_move() which is called in phase 2 after every call of move_rat() by the bots
    for phase 2, we choose location with max prob from cells that don't already have the bot 
        (This was happening automatically with beep!=2 when rat was stationary)
"""
...
def move_rat(grid, rat_pos):
    # Moves the rat to a random open cell from the four cardinal directions.
    # A cell is considered open for the rat if it is OPEN or contains a BOT.
    size = grid.shape[0]
    directions = [(-1, 0), (1, 0), (0, -1), (0, 1)]
    candidates = []
    for a, b in directions:
        i, j = rat_pos[0] + a, rat_pos[1] + b
        if 0 <= i < size and 0 <= j < size:
            if grid[i, j] != BLOCKED:
                candidates.append((i, j))
    if not candidates:
        raise Exception("Error: each open cell must also have a candidate open cell")
    new_pos = random.choice(candidates)
    # Grid updates
    if grid[rat_pos] == RAT:
        grid[rat_pos] = OPEN
    elif grid[rat_pos] == BOT_RAT:
        grid[rat_pos] = BOT
    if grid[new_pos] == BOT:
        grid[new_pos] = BOT_RAT
    elif grid[new_pos] == OPEN:
        grid[new_pos] = RAT
...
def update_Ps_after_rat_move(p_rat, grid):
    size = grid.shape[0]
    new_p_rat = {location : 0.0 for location in p_rat.keys()}
    for (i, j), prob in p_rat.items():
        candidates = []
        for a, b in [(-1, 0), (1, 0), (0, -1), (0, 1)]:
            nbi, nbj = i + a, j + b
            if 0 <= nbi < size and 0 <= nbj < size:
                if grid[nbi, nbj] != BLOCKED:
                    candidates.append((nbi, nbj))
        if not candidates:
            raise Exception(f"No valid moves for cell {(i, j)}")
        # Each open cell distributes its probability equally among open adjacent cells
        p_transition = 1.0 / len(candidates)
        for candidate in candidates:
            new_p_rat[candidate] += prob * p_transition
    total_p = sum(new_p_rat.values())
    if total_p == 0:
        raise Exception("Total probability is zero after update.")
    # Normalising to ensure there are no floating point errors
    for location in new_p_rat:
        new_p_rat[location] /= total_p
    # print("hey!")
    return new_p_rat


def astar_path(grid, bot_pos, max_loc):
    size = grid.shape[0]
...
def bot1(grid, bot_pos, rat_pos, alpha):
    ...
    while len(possible_locations) > 1:
        ...
        neighbor_sensings += 1
        rat_pos, grid = move_rat(grid, rat_pos)
    ...
    rat_pos, grid = move_rat(grid, rat_pos)
    p_rat = update_Ps_after_rat_move(p_rat, grid)
    while True:
        candidates = {location : P for location, P in p_rat.items() if location != bot_pos}
        max_loc = max(candidates, key=candidates.get)
        path, path_found = astar_path(grid, bot_pos, max_loc)
